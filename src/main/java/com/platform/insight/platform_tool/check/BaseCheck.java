...
public class BaseCheck extends ServiceUtils implements PlatformCheck {
    ...
    public Map<String, Object> check(Map<String, Object> actual, PlatformTemplate finalTemplate, boolean is_http) {
        ...
        //是否需要登陆,为false 不要登陆
        if (finalTemplate.isRequireLogin()) {
        ...
        if (finalTemplate.isHttpNotSupport(is_http)) {
        ...
        TemplateItemArr default_fields = finalTemplate.getDefaultFields();
        if (default_fields == null) {
            finalTemplate.put(Const.DEFAULT_FIELDS, new TemplateItemArr());
        }
        Map<String, Object> actual_fields = (Map<String, Object>) actual.get(Const.FIELDS);
        TemplateItemArr template_fields = finalTemplate.getFields();

        Map<String, Object> map = checkFields(actual_fields, template_fields, finalTemplate);
        //检查状态是否成功
        String msg = null;
        if (!isSuccess(map)) {
            return map;
    ...
    protected Map<String, Object> checkFields(Map<String, Object> actual, TemplateItemArr fields, PlatformTemplate template) {
        Map<String, Object> data = new HashMap<>();
        for (int i = 0; i < fields.size(); i++) {
            TemplateItemObj templateItemObj = fields.getTemplateItemObj(i);
//            JSONObject jsonObject = fields.getJSONObject(i);

            String field = templateItemObj.getField_1();
            ...
            try {
              templateItemObj.getField_3RuleValue(template);

            } catch (RuleTypeException e) {
                String errorRuleFormatInfo = templateItemObj.getErrorRuleFormatInfo(e.getMessage());
                template.log(errorRuleFormatInfo);
                return ResultUtils.getResultMap(false,errorRuleFormatInfo);
